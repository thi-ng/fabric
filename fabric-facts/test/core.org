...
#+BEGIN_SRC clojure :noweb-ref tests
  (deftest test-query-join3
    (let [g   (ff/fact-graph)
      ...
      (f/remove-from-graph! jq3 g)
      (f/remove-from-graph! jq4 g)
      (is (== (+ 3 4) (count (f/vertices g))))))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref tests
  (deftest test-paths
    (let [g   (ff/fact-graph)
      ...
      (warn :pq5 @pq5)
      (warn :pq6 @pq6)
      (f/remove-from-graph! pq1 g)
      (f/remove-from-graph! pq2 g)
      (f/remove-from-graph! pq3 g)
      (f/remove-from-graph! pq4 g)
      (f/remove-from-graph! pq5 g)
      (f/remove-from-graph! pq6 g)
