                  ...
                  {::f/collect-fn
                   (fn [vertex]
                     (let [sig-map (f/signal-map vertex)
                           a       (sig-map lhs-id)
                           b       (sig-map rhs-id)
                           bvars   (accumulate-result-vars b)
                           _       (info :minus-bvars bvars)
                           res     (reduce-kv
                                    (fn [res k v]
                                      (reduce
                                       (fn [res v] (set (remove (fn [r] (= (r k) v)) res))) res v))
                                    a bvars)]
...
| =:filter=    | predicate fn         | none      | keep only result maps for which fn returns truthy    |
| =:limit=     | number               | none      | limit results to max. N items                        |
| =:order=     | key fn               | none      | sort results by given key fn                         |
| =:group-by=  | group fn             | none      | group results by given fn                            |
| =:aggregate= | aggregation fn       | none      | inject new qvars of aggregated values                |
