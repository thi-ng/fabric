  ...
  (defn collect-alias-index
    ...
    (let [rf (fn [[index aliases :as acc] [op vid t]]
               (let [x               (nth t idx)
                     ...
                     aliases         (if (props p)
                                       (case op
                                         :add    (let [_ (warn idx :unify s o :pred p)
                                                       aliases (uf/union aliases s o)
                                                       facts' (cond
                                                                (props o) (into #{} (filter #(= s (nth % 1))) (facts g))
                                                                (props s) (into #{} (filter #(= o (nth % 1))) (facts g))
                                                                :else nil)]
                                                   (warn idx :unify-facts facts')
                                                   (reduce
                                                    (fn [acc [s _ o]]
                                                      (warn idx :unify s o)
                                                      (uf/union acc s o))
