 ...
 - [[#namespace-thingfabricfactscore][Namespace: thi.ng.fabric.facts.core]]
     ...
     - [[#core-types--concepts][Core types & concepts]]
         - [[#facts][Facts]]
             ...
             - [[#convertingimporting-facts-from-other-sources][Converting/importing facts from other sources]]
         ...
         - [[#index-vertices][Index vertices]]
             ...
             - [[#alias-index-vertex-fact-triples-only][Alias index vertex (fact triples only)]]
         - [[#fact-transformation][Fact transformation]]
     - [[#queries][Queries]]
         - [[#overview][Overview]]
         - [[#highlevel-query-declaration][Highlevel query declaration]]
         - [[#example][Example]]
...
#+BEGIN_SRC clojure :noweb-ref protos
  ...
  (defprotocol ICache
    (cached [_ k] [_ type k])
    (cache! [_ k v] [_ type k v])
    (expire! [_ k] [_ type k]))

  (defprotocol IIndexSignal
    (index-signal-handler [_]))

  (defprotocol IFactIndex
    (fact-indices [_])
    (add-index-selections! [_ g pattern]))
...
A fact is best understood as a basic statement of truth, based on the
English SPO sentence structure: Facts are simply vectors of (currently
3 or 4) items, e.g. =[subject predicate object]= (for triples) and
stored in custom vertices. A =FactVertex= is more lightweight than a
standard =Vertex=, configured to never collect (facts are immutable)
and signal only once when added (or removed) from the graph.

(Almost) any Clojure value (although you should avoid lazy-seqs) can
be used as fact sub-item, though in most cases you'll want to use
strings, keywords, symbols, UUIDs or numbers.
...
relationships, but cannot be directly used as facts themselves. The
function =map->facts= recursively converts a given hashmap into a
lazy-seq of facts (*conceptually works for triples only*).

Conversion rules:

- Top-level keys are used as subjects and MUST each have a map as
  value, where their keys are used as predicates and their values as
  objects.
- Clojure maps in object position are translated into UUID strings in
  order to allow the nested map to become a subject itself. In RDF
  terms, this is equivalent to [[http://www.w3.org/TR/rdf11-concepts/#section-blank-nodes][BNodes]].
- Sequential types (vectors, lists) in object position cause multiple
  ...
  items in the object sequence is a map, it will be treated as BNode
  as described in previous rule

The following example demonstrates all these rules:

#+BEGIN_SRC clojure
  ;; this example uses common RDF/Linked Data vocabularies to express
...
#+BEGIN_SRC clojure :noweb-ref map-convert
  (defn- bnode-facts
    [s p o]
...
**** Converting/importing facts from other sources

The [[./nt.org][thi.ng.fabric.io.ntriples]] namespace provides a parser to convert
RDF N-Triples into a seq of facts (vectors). Format for other common
Linked Data formats is planned (Turtle, JSON-LD etc.)

**** FactVertex implementation

As with the default =Vertex= type defined in the /[[../../fabric-core/src/core.org#vertex][fabric-core]]/ module,
...
Facts are stored as vertices in a =FactGraph= instance which has been
configured with a maximum fact length (currently 3 or 4 items per
fact, *all facts in a graph MUST have the same length*). The default
...
for DRY reasons and the =FactGraph= makes heavy use of this.
Intermediate result caching is not just done for performance reasons,
but also to minimize memory usage, since queries often utilize similar
patterns and sub-patterns (e.g. two separate query joins might use the
same underlying pattern as LHS or RHS). Currently the cache is an atom
with a nested map and these top-level keys:
...
#+BEGIN_SRC clojure :noweb-ref graph
  (defrecord FactGraph
      [g index facts cache ftx fact-ctor]
    f/IComputeGraph
    (add-vertex!
      [_ val vspec] (f/add-vertex! g val vspec))
    (add-vertex!
      [_ val vspec ctor] (f/add-vertex! g val vspec ctor))
    (remove-vertex!
    ...
    (add-fact!
      ...
      (let [f' (transform ftx f)]
        (or (@facts f')
            (let [v (f/add-vertex! g f' nil fact-ctor)]
              (f/notify-watches g [:add-fact f])
              (debug :add-fact f f')
              (run! #(f/add-edge! g v % signal-fact :add) (fact-indices index))
      ...
      (let [f' (transform ftx f)]
        (if-let [v (@facts f')]
          (do
            ...
            (run! #(f/add-edge! g v % signal-fact :remove) (fact-indices index))
            (swap! facts dissoc f')
...
#+BEGIN_SRC clojure :noweb-ref graph
  (defn fact-graph
    ...
    :fact-ctor - Fact vertex ctor fn (default fact-vertex)
    :transform - fact transform (default identity-transform)"
    ([]
     (fact-graph {}))
    ([{:keys [graph len index fact-ctor transform]}]
     (assert (or (nil? index)
                 (and (satisfies? IFactIndex index)
                      (satisfies? f/IGraphComponent index))))
     (let [index (or index (new-fact-index))
           g     (map->FactGraph
                  {:index     index
                   :facts     (atom {})
                   :fact-ctor (or fact-ctor fact-vertex)
                   :cache     (atom {})
                   :ftx       (or transform identity-transform)
                   :g         (or graph (f/compute-graph))})]
       (f/add-to-graph! index g)
       g)))
#+END_SRC

*** Fact indexing

The =FactGraph= indexes all facts by their individual terms (e.g.
...
usually /only/ to!) these indices. When queries are added to the
graph, they indirectly attach themselves to these indices (via
intermediate index selection vertices, see [[#queries][Queries]] section below) and
then are notified each time their index selection is changing (during
graph execution only!).

The module currently implements two types of indices:

- literal/plain fact terms
- indexing with dynamic entity aliasing (equivalence/synonym)

Both are described (incl. examples) [[#index-vertices][further below]].

The =FactIndex= caches so called "index selection" vertices, each
selecting a specific key from any of the indices (or from a set of
values, or a wildcard selection). These vertices basically act as
sub-sub-queries of a specific fact component and can be shared by
multiple queries.

**** Implementation

The =FactIndex= needs to be instantiated *before* calling the
=fact-graph= constructor function (or if not specified, a default
config will be used). The =FactIndex= type implements
=IGraphComponent= and is added to a graph during the latter's
construction.

*Therefore, do not share =FactIndex= instances between multiple
graphs!*
...
#+BEGIN_SRC clojure :noweb-ref index
  (defrecord FactIndex
    ...
    (fact-indices
      [_] @indices)
    (add-index-selections!
      [_ g pattern]
      (into []
            (map-indexed
             (fn [i x]
               (let [sel [i x]]
                 (if-let [sel' (@cache sel)]
                   (do (debug :reuse-index-sel sel) sel')
                   (cache! index sel (add-index-selection! g _ sel)))))
            pattern))
    ICache
    (cached
      [_ k] (@cache k))
    (cache!
      [_ k v] (swap! cache assoc k v) v)
    (expire!
      [_ k] (swap! cache dissoc k) nil)
    f/IGraphComponent
    (component-type
      [_] :fact-index)
    (add-to-graph!
      [_ g]
      (->> ctors
           (mapv (fn [ctor] (let [v (ctor g)] (info :ctor ctor :v v) (f/add-vertex! g nil nil v))))
           (reset! indices))
      _)
    (remove-from-graph!
      [_ g] (err/unsupported!))
    (remove-from-graph!
      [_ g parent] (err/unsupported!)))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref index
  (defn new-fact-index
    ":len   - fact length (default 3)
     :index - index vertex ctor fn or seq of ctors"
    ([]
     (new-fact-index {}))
    ([{:keys [len index]
       :or {len 3, index default-index-vertex}}]
     (let [ctors (cond
                   (fn? index)
                   (mapv (fn [i] (index #(nth % i))) (range len))
                   ...
                   :else
                   (err/illegal-arg!
                    ":index must be a ctor fn or seq of ctors"))]
       (map->FactIndex
        {:cache   (atom {})
         :indices (atom {})
         :ctors   ctors}))))
...
Index vertices are automatically created during graph construction as
part of the process of adding the =FactIndex=.

The =IndexVertex= type is a customizable vertex implementation used by
both of the currently available indexing methods (below). The
=IndexVertex='s role is to build a lookup table of individual fact
terms, e.g. all subjects and link each term to a set of matching
=FactVertices=. The diagram in the [[#queries][Queries]] section shows how these
vertices are used.

Apart from optional configuration, and unless special indexes are
required, users don't ever need to deal with them directly, though...
...
an index with an initial =same-as= property like this:
...
The alias merging is implemented on top of a Disjoint set (or
Unionfind) datastructure (defined in the [[http://thi.ng/dstruct][thi.ng/dstruct]] project) and
has the following features and behaviors:

- *Order independence* - declaring an equivalence before adding facts
  produces the same result as declaring it after facts have been
  added.

- *Indempodent* - declaring the same equivalence more than once has no
  further effect (apart from if the fact expressing the equivalence
  has been removed meanwhile).

- *Recursive merging* - equivalences can be merged into bigger sets,
  e.g. the 2 facts =[a same-as b] [b same-as c]= will alias A,B and C.
  This too works with declaring new properties defining aliases
  themselves:
...
Since query variables in query patterns are bound to values of
actually existing facts, aliases are *not* resolved for qvars. I.e.
using the above graph/facts as example, the query pattern =[bobby
?pred ?obj]= *will match* the facts about subjects =bob= and =bobby=,
but the pattern =[?subj url "http://bob.com"]= will always only bind
qvar =?subj= to =bob=, and *not* relate any of =bob='s aliases (only
the original facts are matched). That also means the following query
(with filter) will only produce an empty result set:

#+BEGIN_SRC clojure
  {:where [[?subj url "http://bob.com"]] :filter (= ?subj "bobby")}
#+END_SRC

/(Queries are described in more detail in a separate section of this
document.)/
    ...
    (let [props (atom (set props))
          rf    (fn [[index aliases :as acc] [op vid t]]
                  (let [[s p o] t
                        ...
                        (if (@props p)
                          (case op
                            :add    (let [_ (debug :unify s o :pred p)
      ...
      (fn [g]
        (fn [id _ _]
          (IndexVertex.
           ...
           (collect-alias-index g alias-props index))))))
#+END_SRC
...
most items are URIs and often share a common vocabulary. Therefore,
it's common practice in the LD community to specify prefixes for
various, frequently used vocabularies. E.g. We might define a prefix
map and transform like this:
...
#+BEGIN_SRC clojure
  ...
  (def tx (prefix-string-transform prefixes))
...
The transform phase takes such a prefix map and value. If the value is
a string, attempts to find a registered prefix from the map and if
found produces a string of =prefix:name=, else returns the original.

The untransformer too takes the same prefix map and a value. If the
value is a prefix string as produced by the transformation fn and has
a known prefix, returns expanded string or else original value.

The =prefix-string-transform= also implements the =ICache= protocol,
so that new prefixes can be added/removed dynamically.
...
#+BEGIN_SRC clojure :noweb-ref fact-tx
  ...
  (defn prefix-string-transform
    [prefixes]
    (let [prefixes (atom (into (sorted-map) prefixes))
          :sorted  (atom (sort-prefixes @prefixes))
          cache    (atom (hash-map))]
      (reify ITwoWayTransform
        (transform
          [_ x]
          (if (string? x)
            (if-let [v (find @cache x)]
              (key v)
              (reduce
               (fn [acc p]
                 (if (.startsWith ^String x ^String (val p))
                   (let [pre  (first p)
                         name (.substring ^String x (count (val p)))
                         pn   (str pre \: name)]
                     (reduced
                      (if-let [v (find @cache pn)]
                        (key v)
                        (do (swap! cache assoc pn x) pn))))
                   acc))
               x @sorted))
            x))
        (untransform
        ...
        ICache
        (cached
          [_ prefix] (@cache prefix))
        (cache!
          [_ prefix uri]
          (info "register! prefix:" prefix uri)
          (->> (swap! prefixes assoc prefix uri)
               (sort-prefixes)
               (reset! sorted)))
        (expire!
          [_ prefix]
          (info "deregister! prefix:" prefix)
          (->> (swap! prefixes dissoc prefix uri)
               (sort-prefixes)
               (reset! sorted))))))
#+END_SRC
...
provided by this module. In fact, the =FactGraph= and vertices setup
discussed above are really just means to an end - queries.

Queries allow us to treat (parts of) a =FactGraph= like a database to
...
Queries are defined declaratively and attached to the graph as
(potentially nested trees, or at the very least, as vertex chains).
That approach allows for the caching of intermediate results and their
reuse by other queries, thus minimizing work effort.

Queries remain part of the graph until removed again and take part in
every execution cycle of an attached [[../../fabric-core/src/core.org#execution-contexts][execution context]].

*** Highlevel query declaration

The query operations provided in this namespace are considered the
low-level raw building blocks for constructing query trees and are
therefore meant for advanced users. The [[./dsl.org][DSL namespace]] provides a much
more userfriendly and concise approach to define complex queries using
a fully-featured, extensible, hashmap based DSL and expressions. These
query specs are then compiled into query trees using the lowlevel
forms provided here (but also offer many more additional features).
...
#+BEGIN_SRC dot :tangle ../../assets/query-example01.dot :exports none
...
#+BEGIN_SRC clojure :noweb-ref queries
  (defn add-index-selection!
    [g index sel]
    (let [index-v ((fact-indices index) (first sel))
          vertex  (f/add-vertex!
                   g nil
                    ...
                    (remove-from-graph!
                      ...
                      (if (f/none-or-single-user? vertex parent)
                        (do (debug "removing :index-sel" sel)
                            (expire! index ::index-sel sel)
                            (f/disconnect-neighbor! index-v vertex)
                            (f/remove-vertex! g vertex)
                            true)
                        (do (f/disconnect-neighbor! vertex parent)
                            false))))]
      (f/add-edge! g index-v vertex ((index-signal-handler index-v) sel) nil)
      isel))
    ...
    ([g ptx pattern opts]
     (let [pattern (transform ptx pattern)]
       (if-let [q (and (empty? opts) (cached g ::queries pattern))]
         ...
         (let [[s p o] pattern
               sels (add-index-selections! (fact-index g) g pattern)
               acc  (f/add-vertex!
...
=ComputeGraph= of the /fabric-core/ module, a =FactGraph= also emits
the events =:add-fact= or =:remove-fact= whenever a fact is added or
removed. The =add-fact-graph-logger= function below can be used to
...
#+BEGIN_SRC clojure :tangle ../babel/src/thi/ng/fabric/facts/core.cljc :noweb yes :mkdirp yes :padline no
  ...
  (declare default-index-vertex add-index-selection! index-sel-choice)
  (declare map->facts)
