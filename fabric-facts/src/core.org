...
#+BEGIN_SRC clojure :noweb-ref protos
  (defprotocol IFactGraph
    ...
    (register-index-selection! [_ sel v])
    (index-selection-for [_ sel]))
...
(defrecord FactGraph
    [g indices facts queries selections rules]
  f/IComputeGraph
  (add-vertex!
  ...
  (register-index-selection!
    [_ sel v] (swap! selections assoc sel v) v)
  (index-selection-for
    [_ sel] (@selections sel))
  (query-for-pattern
...
#+BEGIN_SRC clojure :noweb-ref queries
  (defrecord FactQueryUnion [id sub-queries result]
    #?@(:clj
         [clojure.lang.IDeref (deref [_] (when result @result))]
         :cljs
         [IDeref (-deref [_] (when result @result))])
...
#+BEGIN_SRC clojure :noweb-ref query-ops
  ...
  (defn add-query-union!
    [g id & queries]
    (f/add-to-graph!
     (map->FactQueryUnion {:id id :sub-queries queries}) g))
  ...
  (defn add-query-filter!
    ...
    (let [tx (comp (mapcat identity) (filter flt))
          fv (f/add-vertex!
              g nil
              {::f/collect-fn      (f/collect-pure (fn [_ in] (sequence tx in)))
               ::f/score-signal-fn f/score-signal-with-new-edges})]
...
#+BEGIN_SRC clojure :noweb-ref query-helpers
  (defn index-selection
    [g sel]
    (if-let [sel-v (index-selection-for g sel)]
      (do (debug :reuse-index sel) sel-v)
      (let [index ((fact-indices g) ([:subj :pred :obj] (first sel)))
        ...
        (register-index-selection! g sel sel-v)
        sel-v)))
