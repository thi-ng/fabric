...
#+BEGIN_SRC clojure :noweb-ref graph
  (defn fact-graph
    ...
    IComputeGraph, fact length and fact transform. If length is not
    ...
    ([g]
     (fact-graph g 3))
    ([g len]
     (fact-graph g 3 identity-transform))
    ([g len tx]
    (map->FactGraph
            ...
            collect-fn (f/collect-pure
                        (fn [_ incoming]
                          (if-let [res (seq (peek incoming))]
                            (into #{} res-tx res)
                            #{})))
            sub-q      (add-query! g identity-transform (raw-pattern _))
            res        (f/add-vertex!
                        g nil
...
#+BEGIN_SRC clojure :noweb-ref query-ops
  ...
  (defn add-param-query!
    ...
    ([g ptx flt pattern]
     (f/add-to-graph!
      (map->ParametricFactQuery {:filter-fn flt :pattern (ptx pattern)}) g)))
  ...
  (defn add-query-union!
    ...
    (let [[ptx a b more] (if (fn? a)
      ...
      (assert b "Requires min. 2 query patterns")
      (f/add-to-graph!
       (map->FactQueryUnion {:sub-queries (mapv ptx (cons a (cons b more)))}) g)))
