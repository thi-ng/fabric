...
* Contents                                                         :toc_3_gh:
 - [[#namespace-thingfabricfactscore][Namespace: thi.ng.fabric.facts.core]]
     ...
     - [[#queries][Queries]]
         ...
         - [[#query-result-modifiers][Query result modifiers]]
    ...
    ([g join-fn lhs rhs opts]
     (let [lhs-v  (result-vertex lhs)
           ...
           jq     (reify
                    ...
                    (remove-from-graph!
                      [_ g parent]
                      (remove-recursively g result parent [lhs rhs] :remove-query-join)))]
  ...
  (defn add-query-union!
    ...
    (let [tx  (if-let [flt (:filter opts)]
          ...
          q   (reify
                ...
                (remove-from-graph!
                  [_ g parent]
                  (remove-recursively g res parent queries :remove-query-union)))]
    ...
    ([g ptx path-pattern mind maxd]
     ...
     (let [[patterns avars] (resolve-path-pattern path-pattern maxd)
           ...
           pq  (reify
                 ...
                 (remove-from-graph!
                   [_ g parent]
                   (remove-recursively g res parent [q] :remove-path-query)))]
    ...
    (if (= :* select)
      ...
      (let [select (if (sequential? select) select [select])
            ...
            sel    (reify
                     ...
                     (remove-from-graph!
                       [_ g parent]
                       (remove-recursively g res parent [q] :remove-qvar-selection)))]
        (f/add-edge! g (result-vertex q) res f/signal-forward nil)
        sel)))
  ...
  (defn add-query-filter!
    ...
    (let [res (f/add-vertex!
          ...
          flt (reify
                ...
                (remove-from-graph!
                  [_ g parent]
                  (remove-recursively g res parent [q] :remove-query-filter)))]
      (f/add-edge! g (result-vertex q) res f/signal-forward nil)
      flt))
  ...
  (defn add-query-group-by!
    ...
    (let [res (f/add-vertex!
          ...
          grp (reify
                ...
                (remove-from-graph!
                  [_ g parent]
                  (remove-recursively g res parent [q] :remove-group-by)))]
      (f/add-edge! g (result-vertex q) res f/signal-forward nil)
      grp))
