...
#+BEGIN_SRC clojure :noweb-ref queries
  (defn add-param-query!
    ...
    ([g ptx pattern opts]
     (add-param-query! g ptx identity pattern opts))
    ([g ptx flt pattern opts]
     (let [pattern (transform ptx pattern)]
       (if-let [q (cached g ::queries pattern)]
         ...
         (let [qvars?  (mapv qvar? pattern)
               ...
               res-tx  (comp res-tx (take (or (:limit opts) MAX_LIMIT)))
                   ...
                   {::f/collect-fn
                    (fn [vertex]
                      (let [sig-map (f/signal-map vertex)
                        ...
                        (f/set-value! vertex (join-fn a b limit))))
               ...
               {::f/collect-fn
                (fn [vertex]
                  (let [subs (vals (f/signal-map vertex))]
                    (f/set-value! vertex (reduce #(if (seq %2) (into % %2) %) subs))))
                ::f/score-signal-fn f/score-signal-with-new-edges})
          q   (reify
                #?@(:clj
                     [clojure.lang.IDeref (deref [_] @res)]
             ...
             (reduce
              (fn [ret x]
                (let [found (idx (select-keys* x ks))]
                  (if found
                    (if (<= (vreset! cnt (+ @cnt (count found))) limit)
                      (reduce #(conj! % (conj %2 x)) ret found)
                      (reduced (reduce #(conj! % (conj %2 x)) ret (take (- @cnt limit) found))))
                    ret)))
              (transient #{}) s)))
          #{}))
      #{}))
  ...
  (defn- collect-basic-query-results
    ...
    (let [ftx    (fact-transform g)
          agg-tx (comp (map #(f/vertex-for-id g %))
                       (clojure.core/filter (or filter identity))
                       (take (or limit MAX_LIMIT))
                       (map deref)
                       (map #(untransform ftx %)))]
      (f/collect-pure
       (fn [_ incoming]
