...
* Contents                                                         :toc_3_gh:
 - [[#namespace-thingfabricfactscore][Namespace: thi.ng.fabric.facts.core]]
     ...
     - [[#core-types--concepts][Core types & concepts]]
         ...
         - [[#todo-fact-transformation][TODO Fact transformation]]
...
*** TODO Fact transformation

The =IFactTransformer= protocol allows the graph to invisibly store
transformed facts, possibly in more compact form than specified by the
user. When a fact is added, the =transform= method of the transformer
...
This is a null transform (NOP), used when no fact transformer is given
for a graph.

#+BEGIN_SRC clojure :noweb-ref fact-tx
  (defn identity-transformer
    []
    (reify IFactTransformer
      (transform [_ x] x)
      (untransform [_ x] x)))
#+END_SRC
...
This transformer is useful when dealing with Linked Data (RDF), where
...
define a prefix map and transformer like this:
...
#+BEGIN_SRC clojure
  ...
  (def tx (prefix-transformer prefixes))
...
The transformer takes such a prefix map and value. If value is a string,
attempts to find prefix from map and if found returns a 2-elem vector
of =[prefix name]=, else original.
...
*Important note:* This transformer only applies to single fact
 sub-items, to apply it to full facts, wrap it in =fact-transformer=
 (described below).

#+BEGIN_SRC clojure :noweb-ref fact-tx
  (defn prefix-transformer
    [prefixes]
    (reify IFactTransformer
      (transform
        [_ x]
...
This transformer too applies to fact sub-items only (see note in
previous section) and maps values to/from unique index IDs.

#+BEGIN_SRC clojure :noweb-ref fact-tx
  (defn global-index-transformer
    []
    (let [index (atom {:fwd {} :rev {} :id 0})]
      (reify IFactTransformer
        (transform
          [_ x]
          (if-let [x' ((@index :fwd) x)]
            x'
            (let [id (:id @index)]
              (swap! index
                     (fn [idx]
...
An arbitrary number of transformers can be composed using the functions below.

- =compose-transformers= applies the given transformers serially (via
  =reduce=) and in reverse order for =untransform=.

- =fact-transformer= applies 3 or 4 transformers in parallel (one per
  fact component)

#+BEGIN_SRC clojure :noweb-ref fact-tx
  (defn compose-transformers
    [& transformers]
    (let [rtx (reverse transformers)]
      (reify IFactTransformer
        (transform [_ x]
          (reduce #(transform %2 %) x transformers))
        (untransform [_ x]
          (reduce #(untransform %2 %) x rtx)))))

  (defn fact-transformer
    ([txs txp txo]
     (reify IFactTransformer
       (transform [_ fact]
...
#+BEGIN_SRC clojure :noweb-ref graph
  (defn fact-graph
    ...
    IComputeGraph, fact length and fact transformer. If length is not
    specified, fact triples are used. Note that currently only triples
    or quads are supported! (len = 3 or 4)."
    ([]
     (fact-graph (f/compute-graph)))
    ([g]
     (fact-graph g 3))
    ([g len]
     (fact-graph g 3 (identity-transformer)))
    ([g len tx]
    (map->FactGraph
...
This function (and =fact-verifier= below) are extensible mechanism to
allow the graph to support facts of varying lengths and queries over
them. By default this module only supports fact triples (len=3) or
quads (len=4).
