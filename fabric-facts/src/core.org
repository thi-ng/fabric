  ...
  (defrecord ParametricFactQuery
    ...
    (add-to-graph!
      ...
      (let [qvars?     (mapv qvar? pattern)
            ...
            res-tx     (comp (map #(if (verify %) (vmap %))) (filter filter-fn))
...
#+BEGIN_SRC clojure :noweb-ref query-helpers
  ...
  (defmulti bind-translator (fn [qvars? pattern] (count pattern)))
  ...
  (defmulti fact-verifier (fn [qvars? pattern] (count pattern)))
