 ...
 - [[#namespace-thingfabricfactscore][Namespace: thi.ng.fabric.facts.core]]
     ...
     - [[#queries][Queries]]
         ...
         - [[#query-helpers][Query helpers]]
             - [[#variable-bindings-injection-multi-method][Variable bindings injection multi-method]]
  ...
  (defn add-join!
    ...
    ([g join-fn lhs rhs opts]
     (let [lhs-v  (result-vertex lhs)
           ...
           _      (info :add-join lhs-id rhs-id opts)
...
| =:filter=    | predicate fn         | keep only result maps for which fn returns truthy |
| =:limit=     | number               | limit results to max. N items                     |
| =:order=     | key fn               | sort results by given key fn                      |
| =:group-by=  | group fn             | group results by given fn                         |
| =:aggregate= | aggregation fn       | inject new qvars of aggregated values             |
    ...
    (let [in-tx (if (and aggregate (not group-by))
          ...
          cfn   (if group-by
                  (let [gfn (transient-group-by-reducer spec group-by)]
                    (if in-tx
                      (fn [_ in] (postprocess-group-by (transduce tx gfn (in-tx (peek in)))))
                      (fn [_ in] (postprocess-group-by (transduce tx gfn (peek in))))))
                  (if in-tx
...
- toplevel keys are used as subjects and MUST each have a map as
  ...
  objects.
- maps in object position are translated into UUIDs in order to allow
  the nested map to become a subject itself. In RDF terms, this is
  equavalent to BNodes.
- sequential types (vectors, lists) in object position cause multiple
  triples with same subject & predicate to be emitted
...
#+BEGIN_SRC clojure
  ...
  ;;  ["b2c1c57e-2967-44fe-8b66-135ec6fa80bf" "foaf:accountServiceHomepage" "http://twitter.com/"]
