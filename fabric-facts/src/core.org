...
#+BEGIN_SRC clojure :noweb-ref graph
  (defn fact-graph
    ...
    ([{:keys [graph len index transform]
     ...
     (map->FactGraph
      {:indices (mapv #(add-index-vertex! graph (index %)) (range len))
       :facts   (atom {})
       :cache   (atom {})
       :ftx     transform
       :g       graph})))
...
#+BEGIN_SRC clojure :noweb-ref index-v
  (defn collect-alias-index
    [props idx]
    (let [rf (fn [[index aliases :as acc] [op vid t]]
               (let [x               (nth t idx)
                     [index aliases] (case op
                                       ...
                                       (do (warn "ignoring unknown index signal op:" op)
                                           acc))
                     aliases         (if (and (not= 1 idx) (props (nth t 1)))
                                       (case op
                                         :add (uf/union aliases (first t) (nth t 2))
                                         :remove (let [[s _ o] t]
                                                   (-> aliases
                                                       (uf/unregister s)
                                                       (uf/unregister o)
    ...
    (fn [idx]
      (fn [id _ _]
        (IndexVertex.
         ...
         (collect-alias-index alias-props idx)))))
#+END_SRC
