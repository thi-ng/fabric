...
#+BEGIN_SRC clojure :noweb-ref query-ops
  ...
  (defn add-query-transitive!
    [g path-pattern mind maxd]
    (let [patterns (resolve-path-pattern path-pattern maxd)
          required (take mind patterns)
          opts     (take (- maxd mind) (drop mind patterns))
          _        (prn :req required :opt opts)
          required (if (seq required)
                     (if (== 1 (count required))
                       (apply add-param-query! g required)
                       (apply add-query-join! g required)))
          opts     (if (seq opts)
                     (if (== 1 (count opts))
                       (apply add-param-query! g required)
                       (apply add-query-join-optional! g opts)))]
      (if (and required opts)
        (add-join! g join-optional required opts)
        (or required opts))))
