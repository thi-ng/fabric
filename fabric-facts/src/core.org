...
#+BEGIN_SRC clojure :noweb-ref queries
  (defn add-query-result!
    [g {:keys [order group-by] :as spec} q]
    (let [tx   (result-transducer spec nil)
          cfn  (cond
                 (and order group-by)
                 (let [gfn (group-by-reducer group-by)]
                   (f/collect-pure
                    (fn [_ in] (transduce tx gfn {} (sort-by order (peek in))))))

                 order
                 (f/collect-pure
                  (fn [_ in] (into [] tx (sort-by order (peek in)))))
...
#+BEGIN_SRC clojure :noweb-ref query-helpers
  ...
  (defn group-by-reducer
    ...
    (fn
      ([] {})
      ([res] res)
      ([res input] (update-in res (grp input) #(conj (or % []) input)))))
