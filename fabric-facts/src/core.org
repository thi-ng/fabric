...
(defrecord FactGraph
  ...
  (clear!
    [_ type k] (cache! assoc-in type k nil)))
#+END_SRC
...
#+BEGIN_SRC clojure :tangle ../babel/examples/query-example01.cljc
  ...
  (def friends (ff/add-query! g :friends '[nil friend nil]))
  (def alice   (ff/add-param-query! g :alice '[alice ?p ?o]))
...
#+BEGIN_SRC clojure :noweb-ref queries
  (defrecord BasicFactQuery [acc result pattern]
    #?@(:clj
    ...
    (add-to-graph!
      ...
      (let [{:keys [subj pred obj]} (fact-indices g)
            [s p o] pattern
            sel-s (index-selection-vertex g [0 s])
            sel-p (index-selection-vertex g [1 p])
            sel-o (index-selection-vertex g [2 o])
        ...
        (f/add-edge! g sel-s acc f/signal-forward nil)
        (f/add-edge! g sel-p acc f/signal-forward nil)
        (f/add-edge! g sel-o acc f/signal-forward nil)
        (f/add-edge! g acc  res f/signal-forward nil)
        (cache! g ::queries pattern this)))
    (remove-from-graph!
      [_ g]
      (clear! g ::queries pattern)
  ...
  (defrecord ParametricFactQuery
    ...
    (add-to-graph!
      ...
      (let [[s p o]    pattern
            ...
            sub-q      (or (cached g ::queries q-pattern) (add-query! g q-pattern))
            res        (f/add-vertex!
                        g nil
...
#+BEGIN_SRC clojure :noweb-ref queries
  (defrecord FactQueryUnion [sub-queries result]
    ...
    (remove-from-graph!
      ...
      (f/remove-vertex! g result)
      (run!
       #(when (sole-user? (result-vertex %) result)
          (f/remove-from-graph! % g))
       sub-queries)))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref query-helpers
  (defn index-selection-vertex
    [g sel]
    (if-let [sel-v (cached g ::index-sel sel)]
      (do (debug :reuse-index-sel sel) sel-v)
      (let [index ((fact-indices g) ([:subj :pred :obj] (first sel)))
            sel-v (f/add-vertex!
                   g nil
                   {::f/score-signal-fn f/score-signal-with-new-edges
                    ::f/collect-fn      (f/collect-pure (fn [_ in] (peek in)))})]
        (f/add-edge! g index sel-v signal-index-select sel)
        (cache! g ::index-sel sel sel-v)
        sel-v)))
