...
In the most trivial case, a single fact has a path length of 1. The
ends of the path may be constants or variables. Variables can not be
used as part of the path (predicates) itself, only the ends.
...
#+BEGIN_SRC clojure :noweb-ref path-query
  (defn add-path-query!
    ([g path-pattern]
    ...
    ([g path-pattern mind maxd]
     ...
     (let [[patterns avars] (resolve-path-pattern path-pattern maxd)
           ...
           req   (if (seq req)
                   (apply (if (== 1 (count req)) add-param-query! add-query-join!) g req))
           opt   (if (seq opt)
                   (apply (if (== 1 (count opt)) add-param-query! add-query-join-optional!) g req))
           res   (if (and req opt)
                   (add-join! g join-optional req opt)
                   (or req opt))
           vs?   (qvar? (first path-pattern))
           vo?   (qvar? (last path-pattern))]
       (cond
         (or (= mind maxd) (and vs? (not vo?)))
         (let [qvars (filter qvar? path-pattern)
               tx    (map #(select-keys* % qvars))
               res'  (f/add-vertex!
                      g #{}
                      {::f/collect-fn (f/collect-pure (fn [_ in] (into #{} tx (peek in))))})]
           (f/add-edge! g (result-vertex res) res' f/signal-forward nil)
           res')

         (and vo? (not vs?))
         (let [?o   (last path-pattern)
               tx   (mapcat #(map (fn [v] {?o v}) (vals %)))
               res' (f/add-vertex!
                     g #{} {::f/collect-fn (f/collect-pure (fn [_ in] (into #{} tx (peek in))))})]
           (f/add-edge! g (result-vertex res) res' f/signal-forward nil)
           res')
...
#+BEGIN_SRC clojure :noweb-ref query-ops
  ...
  (defn add-param-query!
    ...
    ([g ptx flt pattern]
     (f/add-to-graph!
      (map->ParametricFactQuery {:filter-fn flt :pattern (transform ptx pattern)}) g)))
