...
#+BEGIN_SRC dot :tangle ../../assets/facts-overview.dot :exports none :padline no
digraph g {
  ...
  subgraph cluster0 {
    label="thi.ng.fabric.core";
    ctx[label="Execution context"];
    ctx -> ComputeGraph;
  }

  subgraph cluster1 {
    label="thi.ng.fabric.facts";

    f1[label="FactVertex"];
    f2[label="FactVertex"];

    FactIndex[color="#ff6600"];
    iv1[label="IndexVertex", color="#ff6600"];
    iv2[label="IndexVertex", color="#ff6600"];
    iv3[label="IndexVertex", color="#ff6600"];

    isel1[label="Index selection\nvertex", color="#cc0099"];
    isel2[label="Index selection\nvertex", color="#cc0099"];
    isel3[label="Index selection\nvertex", color="#cc0099"];
    isel4[label="Index selection\nvertex", color="#cc0099"];

    q1[label="Query 1", color="#0099cc"];
    q2[label="Query 2", color="#0099cc"];
    q3[label="Query 3", color="#0099cc"];

    qcache[label="Query cache", color="#6699cc"];
    rcache[label="Rule cache", color="#66cc99"];
    scache[label="Selection cache", color="#cc9999"];

    ComputeGraph -> FactGraph -> f1; FactGraph -> f2;
    f1 -> iv1; f1 -> iv2; f1 -> iv3;
    f2 -> iv1; f2 -> iv2; f2 -> iv3;

    FactGraph -> FactIndex -> scache;
    FactGraph -> qcache;
    FactGraph -> rcache;
    ...
    q1 -> rule;
    rcache -> rule[style="dashed"];
    
    scache -> isel1; scache -> isel2; scache -> isel3; scache -> isel4;
    qcache -> q1[style="dashed"]; qcache -> q2[style="dashed"]; qcache -> q3[style="dashed"];
...
selection vertices, see [[#queries][Queries]] section below) and are notified each
    ...
    (add-index-selections!
      ...
      (into []
            (map-indexed
             (fn [i x]
               (let [sel [i x]]
                 (if-let [sel' (get-in @cache [type sel])]
                   (do (debug :reuse-index-sel sel) sel')
                   (add-index-selection! g _ sel)))))
            pattern))
    ICache
    (cached
...
#+BEGIN_SRC clojure :noweb-ref index
  (deftype IndexVertex
    ...
    (connect-to!
      [_ v sig-fn opts]
      ;; TODO refactor opts to include index pattern match
      ;; TODO only connect if this index is compatible w/ desired pattern
      (swap! outs assoc v [sig-fn opts])
      (swap! new-edges inc)
      (debug id "edge to" (f/vertex-id v) "(" (pr-str opts) ") new:" @new-edges)
      _)
...
Unionfind) datastructure and has the following features and behaviors:
...
?obj]= *will match* the facts about subject =bob= and =bobby=, but the
pattern =[?subj url "http://bob.com"]= will always only bind qvar
=?subj= to =bob=, but not relate any of =bob='s aliases (only the
...
/(Queries are described in more detail in the next section of this
document.)/
...
#+BEGIN_SRC clojure :noweb-ref queries
  (defn add-index-selection!
    ...
    (let [index-v ((fact-indices index) (first sel))
      ...
      (cache! index ::index-sel sel isel)))
