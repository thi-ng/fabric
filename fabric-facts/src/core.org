...
#+BEGIN_SRC clojure :noweb-ref path-query
  (defrecord FactPathQuery
      [query result pattern range avars]
    #?@(:clj
         [clojure.lang.IDeref (deref [_] (when result @result))]
         :cljs
         [IDeref (-deref [_] (when result @result))])
    ...
    (add-to-graph!
      ...
      (let [[mind maxd] range
        ...
        (f/add-edge! g (result-vertex query) res f/signal-forward nil)
        (assoc this :result res)))
    (remove-from-graph!
      [_ g] (f/remove-from-graph! _ g nil))
    (remove-from-graph!
      [_ g parent]
      (if (f/none-or-single-user? result parent)
        (do (warn :remove-path-query)
            (f/remove-vertex! g result)
            (f/remove-from-graph! query g result)
            true)
        (do (f/disconnect-neighbor! result parent)
            false))))
#+END_SRC
