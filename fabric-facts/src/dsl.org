...
- =::const= => basically =identity= (default catch-all)
...
The last 3 types can make use of basic type checking of their
arguments and will return =nil= if any of the args violates the type
check. If type checking is disabled (e.g. see =str= below), any
