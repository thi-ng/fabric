...
projection, variable injections, aggregations, sorting etc.
...
The expression language is responsible to compile variable lookups and
result processing expressions into actual Clojure functions. The
language is extensible via its multi-method implementation. See
[[#expressions][section]] further below for details.
...
- =:unique= - ensure that each result map's qvars have unique values
- =:bind= - inject (or overwrite) qvars in result maps (e.g. to format
  dates, concatenate strings etc.). The value of this option must be a
  map of ={?qvar expression..}=
...
#+BEGIN_SRC clojure :noweb-ref dsl
  ...
  (defn sub-query-options
    [opts]
    (-> opts
        (select-keys [:filter :limit :select :unique :bind])
