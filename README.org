...
- Knowledge graphs (triples & quads)
- Fully featured, efficient query engine (variables, joins, optional
  joins, unions, filters, grouping, aggregation)
- Optional map based query DSL (EDN serializable)
- Bounded length path queries (min/max limits)
- Query & rule based fact inferencing (additions & retractions)
- N-Triples parser w/ extensible object literal conversions
- Easy-to-use API

See the [[./fabric-facts/README.org][README]] of the *fabric-facts* module for full feature set.
...
Multi-environment graphs not yet implemented (WIP)
...
Note: This example uses two forms of queries: low-level query node
construction and the more userfriendly and EDN serializable query
specs via the [[./fabric-facts/src/dsl.org][thi.ng.fabric.facts.dsl]] namespace. The query specs are
compiled into these low-level forms.
...
|-----------+------------+------------------------------------------------------------------------------|
|   0.0.214 | 2015-08-29 | DSL/aggregation updates, map->fact conversion, fact transforms, more tests   |
