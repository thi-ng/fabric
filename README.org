...
- Fully featured, efficient query engine (variables, joins, optional
  joins, unions, filters, grouping)
- Bounded length path queries (min/max limits)
- Rulebased fact inferencing (additions & retractions)
...
Note: This example uses two forms of queries: low-level and the more
userfriendly and EDN serializable query specs via the
[[./fabric-facts/src/dsl.org][thi.ng.fabric.facts.dsl]] namespace. The query specs are compiled into
these low-level forms.
...
#+BEGIN_SRC clojure :tangle fabric-facts/babel/examples/example01.cljc :mkdirp yes :padline no
  ...
  (require '[thi.ng.fabric.facts.dsl :as fq])
