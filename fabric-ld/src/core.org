...
#+BEGIN_SRC clojure :noweb-ref model
  (defrecord LDGraphModel [graph ctx bus bus-in]
    comp/Lifecycle
    (start
      [_]
      (info "starting graph model")
      (let [bus-in (async/chan)
            bus    (async/mult bus-in)
            ctx    (f/async-execution-context {:graph (get-graph graph) :result bus-in})
            logtap (async/tap bus (async/chan))]
        (go-loop [] (let [res (<! logtap)] (when res (info :ctx-result res) (recur))))
        (f/execute! ctx)
        (assoc _ :ctx ctx :bus bus :bus-in bus-in)))
    (stop
      [_]
      (info "stop graph model")
      (f/stop! ctx)
      (async/close! bus-in)
      (assoc _ :graph nil :ctx nil :bus nil :bus-in nil))
    IGraphAccess
    (get-graph
      [_] (get-graph graph))
    IGraphModel
...
#+BEGIN_SRC clojure :noweb-ref handler
  ...
  (defn delayed-response-handler
    ...
    (d/->deferred
     (go
       (let [ch  (async/tap (event-bus model) (async/chan))]
         (loop []
           (let [res (<! ch)]
             (if (ready?)
               (do
        ...
        (if-let [qq (:query q)]
          (let [limit  (strf/parse-int limit 10 100)
                ...
                res    (if spec
                         ...
                         @qq)
                body   (into (empty @qq) (comp (drop offset) (take limit)) res)]
            (edn-response
             {:id id
              :count  (count body)
              :total  (count res)
