...
#+BEGIN_SRC clojure :noweb-ref req-validation
  ...
  (def validations
    {:query   {:q         [(v/sequential "must be a vector of sub-query maps")
                           (v/min-length 1)]
               :prefixes  (v/optional (v/map))
               :bind      (v/optional (v/map))
               :values    (v/optional (v/map))
               :aggregate (v/optional (v/map))
               :select    (v/optional (v/alts [(v/equals :*) (v/symbol) (v/sequential)]))
               :filter    (v/optional (v/alts [(v/symbol) (v/sequential)]))
               :group-by  (v/optional (v/alts [(v/symbol) (v/sequential)]))
               :order-by  (v/optional (v/alts [(v/symbol) (v/sequential)]))}
     :opt-int (v/optional (v/pos "must be positive integer"))
     :limit   [(v/optional (v/pos "must be positive integer"))
               (v/optional (v/less-than 1001 "must be <= 1000"))]})
