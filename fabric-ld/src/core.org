...
#+BEGIN_SRC clojure :noweb-ref handler
  ...
  (defmulti api-response (fn [type body rtype] (doto [type rtype] prn)))

  (defmethod api-response :default
    [_ body _] nil)

  (defn- edn-response
    [body] [(pr-str body) "application/edn"])

  (defmethod api-response ["*/*" :default]
    [_ body _] (edn-response body))

  (defmethod api-response ["*/*" :list]
    [_ body _] (edn-response body))
  ...
  (defmethod api-response ["application/edn" :list]
    [_ body _] (edn-response body))
  ...
  (defn- json-response
    ...
    ([body mime]
     [(json/write-str (transform-body* body) :escape-slash false) mime]))

  (defmethod api-response ["application/json" :default]
    [_ body _] (json-response body))

  (defmethod api-response ["application/json" :list]
    [_ body _] (json-response body))
    ...
    (let [qvars (if-let [sel (get-in body [:spec :select])]
          ...
          body  (map
                 (fn [res]
                   (reduce-kv
                    (fn [acc k v]
                      (let [type (if (and (string? v) (.startsWith ^String v "http")) :uri :literal)]
                        (assoc acc k {:type type :value v})))
                    res res))
                 (:body body))]
      (json-response
       {:head {:vars qvars}
  ...
  (defmethod api-response ["text/csv" :default]
    ...
    (if-let [body' (:body body)]
      (csv-response (cons ["subject" "predicate" "object"] body'))
  ...
  (defmethod api-response ["text/csv" :query]
    ...
    (let [qvars (if-let [sel (get-in body [:spec :select])]
          ...
          body  (into [qvars] (map (fn [res] (mapv res qvars))) (:body body))]
      (csv-response body)))

  (defmethod api-response ["text/csv" :list]
    [_ body _] (csv-response (into [(:columns body)] (:body body))))
    ...
    (fn [req]
      (try
        (let [items (reduce-kv
          ...
          (api-response* req {:columns ["id" "spec"] :body items} :list 200))
        (catch Exception e
          (warn e)
          (api-response* req {:body (.getMessage e)} 400)))))
