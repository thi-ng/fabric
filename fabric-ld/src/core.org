...
EDN files/URis and/or restores a previous state from a given graph log
file.
...
|--------------+---------+------------+--------------------------------------------------------|
| =:edn-paths= | vector  | N          | Vector of EDN files/URIs to load facts from on startup |
| =:restore=   | path    | N          | Path to graph log file to restore on startup           |
...
All modifications requests are pushed into a work queue (core.async
channel) and then processed serially by the execution context. Users
can subscribe to execution results by tapping the =result-event-bus=
(using =core.async/tap=).
...
#+BEGIN_SRC clojure :noweb-ref content-neg
  ...
  (defn uri?
    [x] (and (string? x) (re-find #"^(https?|ftp|mailto|urn|isbn)://" x)))
  ...
  (defmethod transform-body java.util.Date
    [x] (.format fmt-iso8601 ^java.util.Date x))
    ...
    (try
      (let [resp  @(http/get
                    ...
                    {:headers {"Accept"     "application/edn,application/n-triples"
                               "User-Agent" (str "thi.ng-fabric/" version)}})
            body  (bs/to-string (:body resp))
            ctype (or (get-in resp [:headers "content-type"])
                      (get-in resp [:headers "Content-Type"]))]
        (condp = ctype
          "application/edn"
          (when-let [facts ((:edn coercions) body)]
            (cond
              (map? facts)        [(ff/map->facts facts)]
              (coll? facts) [facts]
              :else               [nil (str "Returned EDN value is not a collection")]))

          "application/n-triples"
          [(nt/parse-ntriples body)]

          [nil (str "Remote server returned incompatible content type" ctype)]))
      (catch Exception e
        [nil "Error loading facts from remote server"])))
  ...
  (defn list-registry-handler
    ...
    (fn [req]
      (let [items (reduce-kv
        ...
        (api-response* req {:columns ["id" "spec"] :body items} 200))))
