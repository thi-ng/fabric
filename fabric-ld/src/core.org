...
#+BEGIN_SRC dot :tangle ../../assets/ld-overview.dot :exports none
digraph g {
...
- sources listed for =:import= must be pairs of =[type
  path-uri-or-stream]=, supported type keywords are =:edn=,
  =:ntriples= or =:uri=, the latter determines type from URI response's
  Content-type header
  ...
  (defrecord LDGraph [graph prefixes config]
    ...
    (start
      ...
      (let [opts (merge {:transform (ff/combine-transforms prefixes 3)} config)
        ...
        (doseq [[type uri] (:import config)]
          (try
            (let [[facts err] (case type
                                :uri      (load-facts-from-uri uri)
                                :edn      (parse-edn-facts (slurp uri))
                                :ntriples [(nt/parse-ntriples (slurp uri))]
...
| =:buffer= | int           | N          | Buffer size for core.async channel         |

- The default config uses =default-logger= as =:fn= option.
- If unspecified, a buffer size of 1024 is used. For large graphs this
  must be (drastically) increased, especially if the =:restore= option
  is specified for the =LDGraph= component.
  ...
  (defrecord FactLog [config graph logger prefixes]
    ...
    (start
      ...
      (let [log-fn   ((:fn config) config prefixes)
            buf-size (config :buffer 1024)
            logger   (ff/add-fact-graph-logger (get-graph graph) log-fn buf-size)]
...
#+BEGIN_SRC clojure :noweb-ref model
  (defrecord LDGraphModel [graph prefixes ctx queue rbus rbus-in config]
    comp/Lifecycle
    (start
      [_]
      ...
      (let [queue   (async/chan)
            ...
            ctx     (f/async-execution-context
                     {:graph   (get-graph graph)
                      :max-ops (:max-ops config)
                      :result  rbus-in})
  ...
  (defn make-graph-model
    [config]
    (map->LDGraphModel {:config config}))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref model-helpers
  ...
  (defn untransform-pnames-in-query
    [prefixes spec]
    (let [tx   (if-let [prefixes' (:prefixes spec)]
                 (-> {:prefixes (merge (list-registry prefixes) prefixes')}
                     (make-prefix-registry)
                     (comp/start))
                 prefixes)
          spec (reduce
                (fn [acc k]
...
:registry - pname -> uri
:cache - pname -> pname instances

#+BEGIN_SRC clojure :noweb-ref registry
  (defrecord PrefixRegistry [config prefixes sorted cache]
    comp/Lifecycle
    (start
      [_]
      (info "starting prefix registry")
      (let [this (assoc _
                        :prefixes (atom (sorted-map))
                        :sorted   (atom nil)
                        :cache    (atom (hash-map)))]
    ...
    (stop
      ...
      (assoc _ :prefixes nil :sorted nil :cache nil))
    IEntityRegistry
    (register!
      [_ prefix uri]
      (info "register! prefix:" prefix uri)
      (swap! prefixes assoc prefix uri)
      (reset! sorted (ff/sort-prefixes @prefixes))
      _)
    (deregister!
      [_ prefix]
  ...
  (defrecord QueryRegistry [model registry config]
    ...
    (start
      ...
      (let [this (assoc _ :registry (atom (sorted-map)))]
        (run!
         (fn [[id spec]]
           (register! this (name id) (transform-query model spec)))
         (:specs config))
        this))
    (stop
      [_]
...
#+BEGIN_SRC clojure :noweb-ref content-neg
  ...
  (defn- collect-qvars
    [res spec]
    (if-let [sel (:select spec)]
      (if (sequential? sel) sel [sel])
      (into #{} (mapcat keys) res)))
  ...
  (defmethod api-response ["application/sparql-results+json" :query]
    [_ body _]
    (let [qvars (collect-qvars (:body body) (:spec body))
  ...
  (defmethod api-response ["application/sparql-results+json" :query-group]
    [_ body _]
    (let [body (update body :body #(into [] (mapcat identity) (vals %)))]
      (api-response "application/sparql-results+json" body :query)))
  ...
  (defn- sparql-csv-item
    ([x]
     (if (uri? x) (str \< x \>) x))
    ([res qvar]
     (sparql-csv-item (res qvar))))

  ;; http://www.w3.org/TR/sparql11-results-csv-tsv/
  (defmethod api-response ["text/csv" :default]
    [_ body _]
    (if-let [body' (:body body)]
      (let [body' (map (fn [res] (mapv sparql-csv-item res)) body')
            body' (if-let [cols (:columns body)] (cons cols body') body)]
        (csv-response body'))
  ...
  (defmethod api-response ["text/csv" :query-group]
    [_ body _]
    (let [body (update body :body #(into [] (mapcat identity) (vals %)))]
      (api-response "text/csv" body :query)))

  (defmethod api-response ["application/sparql-results+csv" :query]
    [_ body _] (api-response "text/csv" body :query))

  (defmethod api-response ["application/sparql-results+csv" :query]
    [_ body _] (api-response "text/csv" body :query-group))
...
#+BEGIN_SRC clojure :noweb-ref req-validation
  ...
  (def validations
    (let [qvar        (v/validator
                       (fn [_ v] (ff/qvar? v)) "must be a qvar")
          qvar-or-exp (v/optional
                       (v/alts [(qvar) (v/sequential)]
                               "must be qvar or expression"))
          qvar-map    (v/optional
                       ((v/validator
                         (fn [_ v] (and (map? v) (every? ff/qvar? (keys v))))
                         "must be a map with qvar keys")))
          opt-int     (v/optional
                       (v/pos "must be positive integer"))]
      {:query   {:q         [(v/sequential "must be a vector of sub-query maps")
                             (v/min-length 1)]
                 :prefixes  (v/optional
                             ((v/validator
                               (fn [_ v] (and (map? v) (every? string? (keys v)) (every? string? (vals v))))
                               "must be a map with only string keys and values")))
                 :bind      qvar-map
                 :values    qvar-map
                 :aggregate qvar-map
                 :select    (v/optional (v/alts [(v/equals :*) (qvar) (v/sequential)]
                                                "must be :*, qvar or seq of qvars"))
                 :filter    qvar-or-exp
                 :group-by  qvar-or-exp
                 :order-by  qvar-or-exp}
       :qvar    qvar
       :opt-int opt-int
       :limit   [opt-int (v/optional (v/less-than 1001 "must be <= 1000"))]}))
    ...
    (d/->deferred
     (go
       (let [ch  (async/tap (result-event-bus model) (async/chan))]
         (loop [i 0]
           (if (ready?)
             (deliver-delayed-response model ch handler)
             (do (<! ch)
                 (info :delay-recur i)
                 (recur (inc i)))))))))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref handler
  ...
  (defn- inject-routes
    [config model prefixes queries rules routes]
    (sequence
     (comp
      (map
       (fn [handler]
         (info :inject-route-handler handler)
         (handler config model prefixes queries rules)))
      (filter identity))
     routes))
  ...
  (defn app-routes
    ...
    (apply compojure/routes
           (concat
            (inject-routes config model prefixes queries rules (:inject-routes config))
  ...
  (defrecord Handler [config model prefixes queries rules routes]
    ...
    (start [_]
      ...
      (let [routes ((:routes config) config model prefixes queries rules)
            routes ((:middleware config) config routes)]
...
#+BEGIN_SRC clojure :noweb-ref config
  (defn default-config
    []
    {:prefixes {:prefixes {"adms"    "http://www.w3.org/ns/adms#"
...
#+BEGIN_SRC clojure :tangle ../babel/src/thi/ng/fabric/ld/core.clj :noweb yes :mkdirp yes :padline no
  (ns thi.ng.fabric.ld.core
    (:require
     ...
     [aleph.http :as http]
     [byte-streams :as bs]
     [compojure.core :as compojure :refer [GET POST PUT DELETE HEAD ANY]]
