...
#+BEGIN_SRC clojure :noweb-ref handler
  ...
  (defn- inject-routes
    ...
    (->> routes
         (map
          (fn [[method route handler]]
            (info :inject-route method route handler)
            (case method
              :get    (GET route [] (handler model prefixes queries rules))
              :post   (POST route [] (handler model prefixes queries rules))
              :put    (PUT route [] (handler model prefixes queries rules))
              :delete (DELETE route [] (handler model prefixes queries rules))
              :head   (HEAD route [] (handler model prefixes queries rules))
              :any    (ANY route [] (handler model prefixes queries rules))
              (throw
               (IllegalArgumentException.
                (str "Unknown HTTP method: " method "for route: " route))))))
         (filter identity)))
