...
#+BEGIN_SRC clojure :noweb-ref handler
  ...
  (defn validating-handler
    [req params coerce vspec handler]
    (info "-------------------")
    (info :uri (:uri req))
    (info :cookies (:cookies req) :session (:session req))
    (let [params (if coerce
                   (coerce-params coerce params)
                   params)
          [params err] (if vspec
                         (v/validate params vspec)
                         [params])]
  ...
  (defn one-off-query-handler
    ...
    (fn [req]
      (validating-handler
       req (:params req)
       {:spec :edn :limit :int :offset :int}
       {:spec   {:q [(v/sequential "must be a vector of sub-query maps") (v/min-length 1)]}
        :limit  (v/optional (v/pos))
        :offset (v/optional (v/pos))}
    ...
    (fn [req]
      (try
        (let [{:keys [id q prod]} (:params req)
              q    (transform-query model (edn/read-string q))
              prod (edn/read-string prod)
              spec {:q q :prod (set prod)}
              id   (or  (register! rules id spec))]
          (if id
            (api-response* req {:id id :body "Rule registered"} 202)
  ...
  (defn list-facts-handler
    ...
    (fn [req]
      (try
        (api-response* req {:body (get-facts model)} 200)
        (catch Exception e
          (warn e)
          (api-response* req {:body (.getMessage e)} 400)))))
